...
private[iotesters] object setupNcsimBackend {
  def apply[T <: MultiIOModule](dutGen: () => T, optionsManager: TesterOptionsManager): (T, Backend) = {
    ...
    chisel3.Driver.execute(optionsManager, dutGen) match {
      case ChiselExecutionSuccess(Some(circuit), emitted, _) =>
        ...
        // Generate Harness
        val ncsimHarnessFileName = s"${circuit.name}-harness.v"
        val ncsimHarnessFile = new File(dir, ncsimHarnessFileName)
        val vpdFile = new File(dir, s"${circuit.name}.vpd")
        copyVpiFiles(dir.toString)
        genNcsimVerilogHarness(dut, new FileWriter(ncsimHarnessFile), vpdFile.toString)
        assert(
          verilogToNcsim(circuit.name, dir, new File(ncsimHarnessFileName),
            moreNcsimFlags = optionsManager.testerOptions.moreNcsimFlags,
            moreNcsimCFlags = optionsManager.testerOptions.moreNcsimCFlags,
            editCommands = optionsManager.testerOptions.ncsimCommandEdits
          ).! == 0)
        ...
        (dut, new NcsimBackend(dut, command))
...
private[iotesters] class NcsimBackend(dut: MultiIOModule,
                                    cmd: Seq[String],
